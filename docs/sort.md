...
## [136.只出现一次的数字](../136.只出现一次的数字_2.java)

## [169.多数元素](../169.多数元素_4.java)

对数组排序后，$\lfloor\frac{n}{2}\rfloor$ 位置的数必然为众数。

## 164.最大间距

[基数排序](../164.最大间距_1.java)
...
1. [堆排序](../215.数组中的第k个最大元素_2.java)

    - 时间复杂度：$O(n\log n)$，建堆的时间代价是 $O(n)$，删除的总代价是 $O(k\log n)$，因为 $k < n$，故渐进时间复杂为 $O(n+k\log n) = O(n\log n)$。

    - 空间复杂度：$O(k)$。

2. [随机化快速选择](../215.数组中的第K个最大元素_1.java)

    - 时间复杂度：平均为 $O(n)$，最好为 $O(n)$，最差为 $O(n^2)$。

    - 空间复杂度：$O(\log n)$。

3. [中位数的中位数](../215.数组中的第k个最大元素.java)

    - 时间复杂度：$O(n)$。

    - 空间复杂度：$O(n)$。

## [217.存在重复元素](../217.存在重复元素_1.java)

## [242.有效的字母异位词](../242.有效的字母异位词_2.java)

## [260.只出现一次的数字-iii](../260.只出现一次的数字-iii_2.java)

## [268.丢失的数字](../268.丢失的数字_3.java)

## 274.H 指数

1. [快速排序](../274.h-指数_2.java)

    - 时间复杂度：$O(n\log n)$。

    - 空间复杂度：$O(\log n)$。

2. [计数排序](../274.h-指数_1.java)
...
1. [排序+动态规划](../435.无重叠区间_1.java)

2. [排序+贪婪](../435.无重叠区间.java)
...
1. [排序 + 字典树](../1268.搜索推荐系统_1.java)

2. [排序 + 二分查找](../1268.搜索推荐系统.java)
