...
## [5.最长回文子串](../5.最长回文子串_2.java)

## [42.接雨水](../42.接雨水_1.java)

## [44.通配符匹配](../44.通配符匹配.java)

## [53.最大子数组和](../53.最大子数组和_1.java)

## 62.不同路径

1. [时间O(mn)，空间O(mn)](../62.不同路径_1.java)

2. [时间O(mn)，空间O(n)](../62.不同路径_2.java)

## 63.不同路径II

1. [空间O(mn)](../63.不同路径-ii_1.java)

2. [空间O(n)](../63.不同路径-ii_2.java)

## [64.最小路径和](../64.最小路径和.java)

## [72.编辑距离](../72.编辑距离.java)

## [87.扰乱字符串](../87.扰乱字符串.java)

## [91.解码方法](../91.解码方法.java)
...
1. [时间 $O(mn)$，空间 $O(mn)$](../97.交错字符串_1.java)

2. [时间 $O(mn)$，空间 $O(n)$](../97.交错字符串.java)

## 115.不同的子序列

1. [时间 $O(mn)$，空间 $O(mn)$](../115.不同的子序列_1.java)

2. [时间 $O(mn)$，空间 $O(n)$](../115.不同的子序列.java)

## 120.三角形最小路径和

1. [时间 $O(n^2)$，空间 $O(n^2)$](../120.三角形最小路径和_1.java)

2. [时间 $O(n^2)$，空间 $O(n)$](../120.三角形最小路径和.java)

## [121.买卖股票的最佳时机](../121.买卖股票的最佳时机.java)

## 122.买卖股票的最佳时机-ii

1. [时间 $O(n)$，空间 $O(n)$](../122.买卖股票的最佳时机-ii_3.java)

2. [时间 $O(n)$，空间 $O(1)$](../122.买卖股票的最佳时机-ii_2.java)

## 123.买卖股票的最佳时机-iii

1. [时间 $O(n)$，空间 $O(n)$](../123.买卖股票的最佳时机-iii_1.java)

2. [时间 $O(n)$，空间 $O(1)$](../123.买卖股票的最佳时机-iii.java)

## [131.分割回文串](../131.分割回文串.java)

动态规划$\rightarrow$回溯构造解

## 132.分割回文串-ii

1. [动态规划$\rightarrow$回溯构造解](../132.分割回文串-ii_2.java)`超时`

2. [动态规划$\rightarrow$广搜构造解](../132.分割回文串-ii_1.java)`超时`

3. [两次动态规划](../132.分割回文串-ii.java)

## [139.单词拆分](../139.单词拆分.java)

## 152.乘积最大子数组

1. [时间 $O(n)$，空间 $O(n)$](../152.乘积最大子数组_1.java)

2. [时间 $O(n)$，空间 $O(1)$](../152.乘积最大子数组.java)
...
1. [时间 $O(mn)$，空间 $O(mn)$](../174.地下城游戏_1.java)

2. [时间 $O(mn)$，空间 $O(n)$](../174.地下城游戏.java)

## 188.买卖股票的最佳时机-iv

1. [时间 $O(nk)$，空间 $O(nk)$](../188.买卖股票的最佳时机-iv_1.java)

2. [时间 $O(nk)$，空间 $O(k)$](../188.买卖股票的最佳时机-iv.java)

## 198.打家劫舍

1. [时间 $O(n)$，空间 $O(n)$](../198.打家劫舍_1.java)

2. [时间 $O(n)$，空间 $O(1)$](../198.打家劫舍.java)

## 213.打家劫舍-ii

1. [时间 $O(n)$，空间 $O(n)$](../213.打家劫舍-ii_1.java)

2. [时间 $O(n)$，空间 $O(1)$](../213.打家劫舍-ii.java)

## [214.最短回文串](../214.最短回文串_4.java)

## 221.最大正方形

1. [时间 $O(mn)$，空间 $O(mn)$](../221.最大正方形_1.java)

2. [时间 $O(mn)$，空间 $O(n)$](../221.最大正方形.java)
...
1. [时间 $O(\log n)$，空间 $O(\log n)$](../233.数字1的个数_1.java)

2. [时间 $O(\log n)$，空间 $O(1)$](../233.数字1的个数.java)

## 238.除自身以外数组的乘积

1. [借助答案空间 $O(n)$](../238.除自身以外数组的乘积_1.java)

2. [不借助答案空间](../238.除自身以外数组的乘积.java)

## [241.为运算表达式设计优先级](../241.为运算表达式设计优先级.java)

## [264.丑数-ii](../264.丑数-ii.java)

使用三个指针记录当前最小的分别为 $\{2,3,5\}$ 倍数的丑数位置，每次将三个指针分别乘以 $2,3$ 和 $5$，取乘积最小的数作为下一个丑数 $a$，并把所得乘积等于 $a$ 的指针的值都 $+1$，从而排除掉重复的丑数。

## [279.完全平方数](../279.完全平方数_1.java)

$$f[i]=1+\min_{j=1}^{\lfloor\sqrt{i}\rfloor}{f[i-j^2]}$$

## [300.最长递增子序列](../300.最长递增子序列_1.java)

## [303.区域和检索-数组不可变](../303.区域和检索-数组不可变.java)

前缀和实现区间求和。

## [304.二维区域和检索-矩阵不可变](../304.二维区域和检索-矩阵不可变.java)
...
1. [时间 $O(n)$，空间 $O(n)$](../309.买卖股票的最佳时机含冷冻期_1.java)

2. [时间 $O(n)$，空间 $O(1)$](../309.买卖股票的最佳时机含冷冻期.java)

## [312.戳气球](../312.戳气球.java)
...
## [313.超级丑数](../313.超级丑数.java)

分析同 [264.丑数-ii](../264.丑数-ii.java)。

## [322.零钱兑换](../322.零钱兑换.java)

## [334.递增的三元子序列](../334.递增的三元子序列_1.java)

## [338.比特位计数](../338.比特位计数.java)

## 343.整数拆分

1. [动态规划 + 枚举](../343.整数拆分_2.java)

    - 时间复杂度 $O(n^2)$。

    - 空间复杂度 $O(n)$。

2. [动态规划 + 优化枚举](../343.整数拆分_1.java)
...
1. [时间 $O(m\times n)$，空间 $O(m\times n)$](../392.判断子序列_2.java)

2. [时间 $O(m\times|\Sigma|+n)$，空间 $O(m\times|\Sigma|)$](../392.判断子序列.java)，其中 $|\Sigma|$ 表示字符集大小，$m$ 表示字符串长度。如果是计算 $k$ 个平均长度为 $n$ 的字符串是否为 $t$ 的子序列，则时间复杂度为 $O(m\times|\Sigma|+k\times n)$。

## [427.建立四叉树](../427.建立四叉树_1.java)

前缀和算矩阵区间和，乘法及和比较判断子矩阵元素是否相同。

## [435.无重叠区间](../435.无重叠区间_1.java)
...
## [509.斐波那契数](../509.斐波那契数_2.java)

## [724.寻找数组的中心下标](../724.寻找数组的中心下标.java)

## 714.买卖股票的最佳时机含手续费

1. [时间 $O(n)$，空间 $O(n)$](../714.买卖股票的最佳时机含手续费_2.java)

2. [时间 $O(n)$，空间 $O(1)$](../714.买卖股票的最佳时机含手续费_1.java)

## 746.使用最小花费爬楼梯

1. [时间复杂度 $O(n)$，空间复杂度 $O(n)$](../746.使用最小花费爬楼梯_1.java)

2. [时间复杂度 $O(n)$，空间复杂度 $O(1)$](../746.使用最小花费爬楼梯.java)

## [768.最多能完成排序的块-ii](../768.最多能完成排序的块-ii_1.java)

## 790.多米诺和托米诺平铺

1. [时间复杂度 $O(n^2)$，空间复杂度 $O(n)$](../790.多米诺和托米诺平铺_2.java)
...
2. [时间复杂度 $O(n)$，空间复杂度 $O(1)$](../790.多米诺和托米诺平铺_1.java)
...
1. [时间复杂度 $O(n)$，空间复杂度 $O(n)$](../918.环形子数组的最大和_2.java)
...
2. [时间复杂度 $O(n)$，空间复杂度 $O(1)$](../918.环形子数组的最大和.java)
...
1. [时间复杂度 $O(n)$，空间复杂度 $O(n)$](../1137.第-n-个泰波那契数_1.java)

2. [时间复杂度 $O(n)$，空间复杂度 $O(1)$](../1137.第-n-个泰波那契数.java)

## [1143.最长公共子序列](../1143.最长公共子序列.java)

## [1480.一维数组的动态和](../1480.一维数组的动态和.java)

## 1493.删掉一个元素以后全为 1 的最长子数组

1. [保存左部到当前位置最长的全 1 数组长度](../1493.删掉一个元素以后全为-1-的最长子数组_2.java)

    - 时间复杂度 $O(n)$。

    - 空间复杂度 $O(n)$。

2. [双 dp 优化](../1493.删掉一个元素以后全为-1-的最长子数组_1.java)
